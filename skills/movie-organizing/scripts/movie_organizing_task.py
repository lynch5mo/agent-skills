...
VERSION = "1.3.5"
FIXED_STEPS = ("verify_install", "scope_lock", "inventory", "naming_contract", "preprocess", "exception_resolution", "core_gate", "dedupe_gate", "cleanup_final_audit")
...
PREPROCESSOR, AUDIT, SLOWPATH, SKILL_DIR = (SCRIPT_DIR / "movie_organizing_preprocessor.py", SCRIPT_DIR / "movie_organizing_audit.py", SCRIPT_DIR / "movie_organizing_slowpath.py", SCRIPT_DIR.parent)
...
def _is_formal(value: Dict[str, Any]) -> bool:
    return value.get("schema") in {PRE_RESULT_SCHEMA, SLOW_RESULT_SCHEMA} and (value.get("mode") == "verify" or (value.get("mode") == "apply" and value.get("dry_run") is not True))
...
def _blocking_failure(records: List[Tuple[int, Path, Dict[str, Any]]]) -> Optional[Dict[str, Any]]:
    plans = _latest(records, lambda value: _is_pre_plan(value) or value.get("schema") == SLOW_PLAN_SCHEMA)
    ...
    if naming is not None and not slow_is_newer:
        ...
        if value.get("mode") == "verify" and value.get("status") == "PASS":
            ...
            if int(summary.get("deferred_action_units", 0) or 0) > 0:
                return "preprocess", _pre("plan", root), "bounded batch still has deferred units; create a fresh plan"
            if audit is not None and audit[0] > result[0]:
                return _audit_state(root, audit)
